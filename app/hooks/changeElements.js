...
export const useChangeElements = () => {
    ...
    const [textP, setTextP] = useState('¿Qué dices?')
    ...
    const options = ['Segura?', 'Segurisima?', 'Estas completamente segura?', 'Deberitas, deberitas?', 'No te arrepentiras?', 'Pero si estas segura?', 'No hay vuelta atras', 'No hay devoluciones', 'No hay garantias', 'No hay reembolsos', 'No hay cambios', 'No hay nada', 'No hay', 'Que no hay']
    ...
    const handleButtonYes = () => {
        ...
        setTitle('Eres mi San Valentin!')
        setShowButtons(false)
        confetti()
    }

    return { handleButtonNo, handleButtonYes, textP, image, title, showButtons }
}